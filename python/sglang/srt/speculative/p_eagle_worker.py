...
def _draft_sample_with_dsl_kernel(
    ...
    for k in range(K):
        ...
        for v_start in range(0, vocab_size, BLOCK_V):
            ...
            # Find block's top-1 and top-2
            block_max = tl.max(logits_block, axis=0)
            block_argmax = tl.argmax(logits_block, axis=0) + v_start

            # Find block's second-highest value by masking out the block argmax
            block_logits_masked = tl.where(
                tl.arange(0, BLOCK_V) == (block_argmax - v_start),
                -1e9,
                logits_block
            )
            block_second = tl.max(block_logits_masked, axis=0)

            # Update global max1/max2/argmax
            if block_max > max1:
                # New global max found
                if block_argmax == argmax:
                    # Same token, just update max1
                    max1 = block_max
                else:
                    # Different token: old max1 becomes candidate for max2
                    if max1 > max2:
                        max2 = max1
                    max1 = block_max
                    argmax = block_argmax
                    # Block's second might also be a candidate for global max2
                    if block_second > max2:
                        max2 = block_second
            elif block_max > max2:
                # Block max is not global max1, but could be max2
                max2 = block_max

            # Block's second value could also be max2
            if block_second > max2 and block_second < max1:
                max2 = block_second
        ...
        # Greedy sample: argmax from reduction above
        token = argmax

        # Log-probability via numerically-stable log-sum-exp over full vocab
        # (needed for rejection sampling in the verify step)
        # Accumulate sum_exp across all blocks using the global max1
        sum_exp = 0.0
        for v_start in range(0, vocab_size, BLOCK_V):
            v_offs = v_start + tl.arange(0, BLOCK_V)
            v_mask = v_offs < vocab_size
            logits_block = tl.load(base_ptr + v_offs, mask=v_mask, other=-1e9)
            # Compute exp(logit - max1) for numerical stability, respecting mask
            exp_block = tl.where(v_mask, tl.exp(logits_block - max1), 0.0)
            sum_exp += tl.sum(exp_block, axis=0)
        log_sum_exp = max1 + tl.log(sum_exp)
...
class PEAGLEDSLWorker(PEAGLEWorker):
    ...
    def _build_draft_tree_from_logits_and_dsl(
        ...
        for i in range(K):
            ...
            # Use DSL-provided draft tokens and scores instead of recomputing
            # Check if DSL produced valid outputs for this step
            has_dsl_output = (draft_scores[:, i] > exited_sentinel).any()

            if i == 0:
                # First position: use topk_p/topk_index from spec_info (precomputed by target model)
                step_topk_p, step_topk_index = topk_p, topk_index
            elif has_dsl_output:
                # DSL outputs available: use draft_tokens and draft_scores
                # Convert DSL greedy tokens to topk format by placing them at position 0
                # and filling remaining topk-1 positions from softmax
                step_topk_index = torch.zeros(
                    batch_size, self.topk, dtype=torch.long, device=step_logits.device
                )
                step_topk_p = torch.zeros(
                    batch_size, self.topk, dtype=torch.float32, device=step_logits.device
                )

                # Place DSL tokens at index 0 of topk
                step_topk_index[:, 0] = draft_tokens[:, i].long()

                # Compute probabilities for DSL tokens
                probs = torch.softmax(step_logits, dim=-1)
                dsl_token_probs = probs.gather(1, draft_tokens[:, i].long().unsqueeze(1)).squeeze(1)
                step_topk_p[:, 0] = dsl_token_probs

                # Fill remaining topk-1 positions with next-best tokens from softmax
                if self.topk > 1:
                    remaining_topk_p, remaining_topk_index = fast_topk(probs, self.topk, dim=-1)
                    # Copy positions 1..topk from fast_topk result
                    # (position 0 is already filled with DSL token)
                    step_topk_p[:, 1:] = remaining_topk_p[:, 1:]
                    step_topk_index[:, 1:] = remaining_topk_index[:, 1:]
            else:
                # No DSL output: fall back to computing topk from logits
                probs = torch.softmax(step_logits, dim=-1)
                step_topk_p, step_topk_index = fast_topk(probs, self.topk, dim=-1)
